---
status: accepted (amends the execution-platform clause of ADR-0021 and the stuck clause of ADR-0001; makes ADR-0027's Compose self-healing concrete; the admission pause and its resume are made concrete by ADR-0039; #95 fixes how long the first restart's 需留意 lasts)
---
...
- **A Connect restart fails its running boxes at once.** After a restart, Sources re-read everything since their last offset commit, and Sinks (`insert`, `pk.mode=none`) redeliver. So continuity is already disproved (ADR-0006).
  ...
  - **How DBX sees it: a restart marker (重启标记) in the worker's memory** ([#79](https://github.com/liumingjian/dbx/issues/79), lab-confirmed in [#90](https://github.com/liumingjian/dbx/issues/90); details in `docs/research/connect-restart-detection.md`). DBX plants it with `PUT /admin/loggers/dbx.incarnation` `{"level":"ERROR"}` at worker scope (KIP-976, Kafka ≥ 3.7). It plants before a run admits its first box and after every detected restart, and retries until the PUT returns 200. A new JVM never has the marker, so a restart shorter than a poll still shows on the next successful check.
  - The check is `GET /admin/loggers/dbx.incarnation`. It runs on ADR-0005's five-second status poll and once at DBX startup, before ADR-0006's reconciliation. **Restarted** means Connect's own JSON 404 (`Logger dbx.incarnation not found.`), or 200 with `last_modified: null`. Any other answer means **unreachable** (ADR-0021): a connection error, a timeout, a 5xx, or a non-JSON 404 from a REST server whose resources aren't registered yet. A 200 with a non-null `last_modified` and a level other than `ERROR` means someone changed an enclosing log level: the worker did not restart, and DBX re-plants.
  - v1 has no fallback signal. The release pins `cp-kafka-connect:7.9.0` (ADR-0035), so the logger API is always present, and the Connect group member id (`describeClassicGroups`) also changes on session-expiry fencing. DBX therefore doesn't need Kafka clients ≥ 4.0 for this.
...
- **The second restart stops the run's admission.** The first Connect restart in a run turns the runtime condition (运行状况) to 需留意, with the reason *迁移平台发生过重启*, and admission continues. A second restart in the same run turns it to 受阻 and pauses admission. Admission resumes only when the DBA chooses to continue; the alternative is stopping the run. The condition-change record and the diagnostic package carry each restart's time and its OOM evidence. The first restart's 需留意 is not an event with a lifetime but a projection of the fact *this run's restart count is at least one*: it lasts until that run reaches a terminal status, and it is gone once no nonterminal run holds a restart count ([#95](https://github.com/liumingjian/dbx/issues/95)). It never decays on a timer, because ADR-0039's counters do not reset either — a 需留意 that faded on its own would leave the second restart's 受阻 with no build-up — and because a timer would put a clock inside a fold that takes every timestamp from its input.
...
- The lab Compose (`local-env/docker-compose.yml`) now carries both `restart: unless-stopped` and the flag. Crash tests must end the JVM for real: `docker kill` counts as a manual stop, so Docker skips the restart policy ([#90](https://github.com/liumingjian/dbx/issues/90)).
- Release certification (L3/L4, ADR-0022) must prove the marker protocol on the pinned Connect image. A Connect upgrade that drops KIP-976 `last_modified` breaks restart detection.
