...
| `web` | HTTP adapters, the installation-scoped status channel beside `RunProgressSource` (ADR-0016, ADR-0021), and the nonterminal-run query used by upgrade. The status channel is `web`'s one read through an `orchestration` use case rather than `workflow.api.query`, because the latest condition outcome is never persisted (ADR-0021 §Consequences, #95) | — |
