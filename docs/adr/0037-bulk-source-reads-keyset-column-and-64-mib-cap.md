...
- **`tinyint(1)` never qualifies, signed or unsigned** ([#133](https://github.com/liumingjian/dbx/issues/133)). Under the Boolean switch a `tinyint(1)` reaches Connect as `BOOLEAN`, which the connector cannot page by. `tinyint(1) unsigned`, which Connector/J leaves as an integer, is excluded with it: one shape, one rule, and a unique `tinyint` column bounds its table at 256 rows, which the bulk path admits anyway. **Erratum**: spec [#123](https://github.com/liumingjian/dbx/issues/123) justified this exclusion with "a unique `tinyint(1)` column holds at most two non-null rows". That is false — a display width does not bound the value range, so a unique `tinyint(1)` holds up to 256 rows. The exclusion stands on the reason above; no implementation ever relied on the false one.
