...
- **No catalog code of its own.** 准入已暂停 is a state the run holds, not an occurrence, so it has no diagnosis code: a diagnosis carries exactly one primary phase (ADR-0005), and the pause belongs to the run rather than to any unit or box. Its two triggers already ship structured codes — 迁移平台在传输中重启 and 迁移平台无法启动新的读取 — which state what happened, what is affected, and the one action (press 继续迁移). The diagnostic package carries the pause through the run fact's reason, trigger, time, and counters ([#96](https://github.com/liumingjian/dbx/issues/96)).
