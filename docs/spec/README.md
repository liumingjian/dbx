...
Compiling settled every obligation but 29 items, `D-1`–`D-29`, of which every item but `D-25`–`D-26` is now resolved (2 open). Each is listed under **Open items** in the sub-specs it affects and blocks only the slices it names there. They are grouped into eight decision tickets on map #1:
