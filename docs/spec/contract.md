...
8. A `USER` mapping rule overrides an `AUTO` rule. `assemble` resolves that precedence: a `USER` table rename sets the approved target coordinate, and the resolved selected columns and target names are what `dialect`'s `source.queryProjection` renders. `pair.mapIdentifier` never applies a rule. Column prune and rename take effect in the Source query projection, never DDL-only (technical plan §7.1; ADR-0011 §Contract assembly; #123).
9. Approved coordinates, including a dialect-decided rename (`<prefix>_<hash12>`) with its rule, full coordinate, and algorithm version, are frozen in the contract. So is the name of an owned `numeric(20,0)` sequence. `assemble` derives it deterministically within 63 bytes under TP §7.1's rename and collision rules and hands it to `target.ddlPlan`, which never invents a name (#123). No later phase re-derives them (ADR-0008 §Contract and mapping boundary; technical plan §7.1).
