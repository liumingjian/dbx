...
**Read first**: ADR-0036 (module table, purity), ADR-0018 (enforcement, session rule), ADR-0002, ADR-0013, ADR-0031, ADR-0033 §read-ahead, ADR-0039, ADR-0021 §Values, ADR-0019, ADR-0034, ADR-0038, ADR-0024 §Planning, ADR-0022; #89 item 2; ADR-0002 §Box demand (#93). CONTEXT.md terms: Box (箱), Execution signature (执行签名), Admission paused (准入已暂停), Platform memory budget (平台内存预算), Large record table (大记录表), Minimum window (窗口下限), Reference throughput band (参考吞吐带), Estimate unavailable (无法预估), Low confidence (低置信), Reliable (可信).
...
6. Each connection budget = 10% of that database's `max_connections`, clamped to 4–20, less the two reserved per side for DBX's own connections: the gate counts connector connections only, so it is `budget − 2`. `tasks per box` = 2 (Source and Sink each at `tasks.max=1`) and a box demands one connection per side. Connect tasks = twice the Connect worker's container-visible CPU count, read at the pre-admission environment check (ADR-0002 §Box demand).
7. M for a box = the largest exact row byte length among its tables (ADR-0031 §Per-box bounds).
8. R per box = `buffer.memory × E + read-ahead + 2 × fetch.max.bytes + max.poll.records × M × X`. The settings come from the box's execution signature and are never re-derived here. Read-ahead is ADR-0033's term. E = 100 for ordinary boxes and 3 for large-record boxes; X = 3; B = 512 MiB; `R_narrow` = 512 MiB, all as versioned constants (ADR-0031 §Platform memory budget, ADR-0033).
...
14. Large-box starvation protection is head-of-line reservation on the cumulative gates (disk, memory): the first unadmitted box in plan order counts as occupied there, so later boxes get only the remainder. One reservation at a time (ADR-0002 §Box demand, ADR-0031).
...
34. Return 无法预估 with a reason, and with bytes transferred of total and current throughput, when elapsed time passes the duration estimate's upper bound or the live range stays unstable. **Unstable**: across the last 6 consecutive 10 s samples, the spread of projected finish instants exceeds 25% of the window's median remaining; return to a range when it holds at or below 25% for a full window. Fewer than 6 samples is never unstable. Both constants are versioned with ADR-0034's (ADR-0019, #93).
...
2. **plan** (1–10) with the box-plan golden set. Blocked by slice 1.
3. **admit** (11–18). Blocked by slice 2.
