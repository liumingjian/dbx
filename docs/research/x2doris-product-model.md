...
按"抄的价值"排序，每条都落到具体控件/交互。

### 7.1 三段式信息架构：系统设置 / 数据源 / 作业中心
整个产品只有三个主入口，且职责互斥：**全局环境（配一次）→ 连接（配 N 次）→ 作业（用 N 次）**。
- 抄法：DBX 左侧导航就三项——「系统设置」（目标 PG、执行参数默认值）、「数据源」（MySQL 连接）、「迁移任务」。**不要**在向导里再问一次连接信息。

### 7.2 目标端全局唯一，不做成作业参数
X2Doris 把 Target Doris 放进系统设置，作业创建全程不再问目标端。
- 抄法：DBX 若确定"一个部署对应一个目标 PG"，就把目标库放进系统设置，向导第一步直接从"选源库"开始，砍掉一整屏。

### 7.3 新建作业时**自动探测连接并当场告警**
Hive 作业创建入口会自动检测连接，失败直接提示"去检查系统设置里的 metastore uris"。
- 抄法：DBX 点「新建迁移任务」时立刻 ping 源库与目标库，失败的错误文案必须**指向修复位置**（"连接失败 → 前往 数据源 > xxx 修改"），而不是抛异常堆栈。

### 7.4 建表意图做成**三个并列按钮**，而不是一个"下一步"+复选框
「跳过建表」/「只建表」/「创建表 & 作业」。
- 这是本报告认为**最值得抄的一个交互**：它把"目标表已经有了 / 我只想先把表建好给别人看 / 我要一把梭"三种真实工作方式变成三个显式出口，用户不需要理解内部状态机。
- 抄法：DBX 审核 DDL 之后给三个按钮——**「跳过建表（表已存在）」「仅建表」「建表并开始迁移」**。DBA 常见的工作方式正是"先建表、给业务方看一眼、明晚再灌数据"，「仅建表」直接命中。

### 7.5 DDL 审核页 = 强制一屏 + 可编辑文本
生成的 DDL 全文摆出来，允许手改后再执行。
- 抄法：DBX 已定方案就是这样。补一条 X2Doris 的做法：**审核页是流程的一步，不是折叠面板**。用户必须看见它才能继续。

### 7.6 字段映射表把"必须做的决策"限制在极少数几列
Hive 场景只有三列需要人管：目标类型、DUPLICATE KEY、DISTRIBUTED BY，其余全自动预填。
- 抄法：DBX 的 MySQL→PG 映射表里，默认全部预填，只把**真正有歧义的字段高亮出来**（如 `tinyint(1)` → `boolean`/`smallint`、`datetime` 精度、无符号整型、`enum`、字符集/排序规则）。**其余字段折叠或灰显**，让 200 字段的表看起来只有 3 个待办。

### 7.7 已知不可用组合直接前置为界面规则
"STRING 不能作为 DUPLICATE KEY，请改为 VARCHAR" 是写进流程里的硬约束。
- 抄法：DBX 把 PG 侧的硬约束（如主键不能是 `text` 上无长度限制的场景、索引长度、`serial` 与自增映射）做成**行内校验 + 一键修复建议**，而不是等建表失败再报错。

### 7.8 迁移时的 WHERE 过滤放在**启动弹窗**而非作业定义里
启动作业时才填"查询条件（仅写 where 之后的逻辑）"。
- 抄法：DBX 可让"本次只迁最近 30 天"这类临时诉求在**启动时**给出，作业定义保持干净可复用。注意 X2Doris 的这个过滤是全局一条件、作用于所选表，DBX 若做多表需明确语义。

### 7.9 危险操作显式标注 + 默认关闭
「清空数据」默认 OFF，文案原文"极度危险的操作，生产数据迁移时，慎重操作"。
- 抄法：DBX 的「迁移前清空目标表」必须默认关闭、红色标注，并在启动确认里回显"将清空 N 张表"。

### 7.10 作业名与标签自动生成
用户不被强制命名。
- 抄法：DBX 任务名默认 `mysql_db → pg_db @ 2026-07-23 14:20`，可改不必填。

### 7.11 O(1) 决策原则
默认路径上的用户操作次数与表数量无关（详见第 5 节）。
- 抄法：DBX 的验收标准可以直接写成一句话——**"迁移 200 张表的默认路径，用户点击次数应与迁移 1 张表相同。"**

---

## 8. 短板与用户抱怨

**证据强度声明**：如第 0 节所述，X2Doris 闭源、无公开 GitHub issue 区，**无法提供 GitHub 用户抱怨的一手引用**。下表的"证据"列区分了三档：**官方原文**（官方文档自己承认/警告）、**文档缺失**（一手来源里查不到，说明能力不存在或未文档化）、**推断**（由已知事实推出，明确标注）。

| # | 短板 | 证据 | 对 DBX 的教训 |
| --- | --- | --- | --- |
| 1 | **失败根因要 ssh 上服务器看 `log/selectdb.out`** | 官方原文（使用手册 + 阿里云文档均如此指引） | 非开发用户过不了这关。DBX 必须把日志与错误摘要搬进界面 |
| 2 | **进度靠手动刷新，且官方警告不要频繁刷新** | 官方原文"切记不要频繁的刷新进度更新按钮" | 暴露后端进度查询代价高。DBX 应服务端聚合表级进度并低频轮询推送 |
| 3 | **排障兜底是"联系官方支持"** | 官方原文 | 说明自助诊断做得不足 |
| 4 | **多表/整库模式下完全看不到 DDL** | 官方原文（"多表使用默认策略，右侧不显示 DDL"） | DBX 应保留"整库选中 + 单表展开审核"的能力 |
| 5 | **没有官方的"整库中对个别表做例外"路径** | 文档缺失 | DBX 的"例外配置"正是补这个洞，是差异化点 |
| 6 | **目标 database 必须手工先建** | 官方原文 | DBX 应自动建 schema/database 或至少提供一键创建 |
| 7 | **Hive 连接信息一半在界面、一半在 YAML**（`application-hive.yaml` 选 metastore/jdbc/dlf） | 官方原文 | DBX 不应把任何连接决策留在配置文件里 |
| 8 | **安装包按 Scala 版本 + 架构分发，需用户去 `$SPARK_HOME/jars` 数版本号** | 官方原文（部署手册） | DBX 应单包/单容器，零版本抉择 |
| 9 | **强依赖 Spark（及 Hive 场景的 Hadoop gateway）** | 官方原文 | 对 MySQL→PG 这种规模，DBX 引入 Spark 是过度工程 |
| 10 | **类型映射规则不可配置、不可导出，无映射规则文档页** | 文档缺失（未找到官方类型对照表页） | DBX 至少应在界面上把"本次使用的映射规则"可视化，便于 DBA 复核与留档 |
| 11 | **已知类型兼容坑**：社区资料记载 Doris→Doris 不支持 bitmap；1.2 系列不支持 DATETIMEV2/DATEV2，2.1.2 之后读这两种类型仍有问题 | 二手（CSDN 转述版本说明，未在官网核实）—— **标注为待核实** | 说明"自动映射"在边缘类型上仍会翻车，DBX 需要一份明确的"不支持类型"清单并在审核页前置提示 |
| 12 | **无增量/断点续传，失败即整作业重跑** | 推断（产品定位为离线全量；界面只有作业级启动与 Max retry） | DBX 的"按表重跑"是实打实的改进 |
| 13 | **无数据校验环节**（迁完不比对行数/校验和） | 文档缺失 | DBX 可考虑迁移后自动行数比对，作为差异化 |
| 14 | **两处官方来源默认口令不一致**（`admin/admin` vs `admin/selectdb`） | 官方原文冲突 | 文档一致性问题，提醒 DBX 文档与产品同源生成 |

---

## 9. 同类工具在"自动建表 + 字段映射"上的对照（加分项，从简）

- **Apache Flink CDC（YAML Pipeline）** — 交互形态是**声明式配置而非向导**，但两个概念对 DBX 的"例外配置"很有借鉴：
  - `route`：用**正则 + `replace-symbol` 占位符**表达整库映射规则，如 `source-table: source_db.\.*` / `sink-table: sink_db.<>`，一条规则覆盖全库，另可为个别表写单独规则并按"首个匹配优先"决定归属。这正是"整库默认 + 个别例外"的干净模型，比 X2Doris 的"多表即放弃审核"高明。
    https://nightlies.apache.org/flink/flink-cdc-docs-master/docs/core-concept/route/
  - `schema.change.behavior`：把结构变更策略做成显式枚举 `exception / evolve / try_evolve / lenient / ignore`，让"遇到不兼容时怎么办"成为用户的一次全局决策而非 N 次弹窗。DBX 可借此思路，把"目标表已存在时怎么办"做成一个全局枚举（报错 / 跳过 / 追加 / 重建）。
    https://nightlies.apache.org/flink/flink-cdc-docs-release-3.2/docs/core-concept/schema-evolution/
  - 自动建表能力："能够基于上游表结构推断并自动创建下游表"，MySQL→Doris 快速上手页给了完整整库同步 YAML。
    https://nightlies.apache.org/flink/flink-cdc-docs-master/docs/get-started/quickstart/mysql-to-doris/
- **CloudCanal** — 把「结构迁移」做成**可独立运行的一档功能**（可只勾选"表结构迁移"而不迁数据），并在任务创建流程中有独立的"库映射 → 表映射 → 列映射/裁剪"三级步骤。对应 X2Doris 的「只建表」按钮，但做成了完整的三级映射向导。
  https://doris.apache.org/zh-CN/docs/2.1/ecosystem/cloudcanal/
  *（注：CloudCanal 官方文档站的列映射细节本次未取到一手页面，此处仅据 Doris 官方生态页与产品页描述，粒度到此为止。）*
- **NineData** — 复制详情页可在目标对象上右键查看源/目标的映射信息；官方强调异构长期运行必然有不兼容场景，因此在"可观测与可干预"上做设计。**未取到一手帮助文档页**，不深入。
  https://www.ninedata.cloud/dbmigration
- **AWS DMS / Airbyte** — 本次未做一手取证，不做断言。

---

## 10. 一页纸结论：喂给"迁移向导旅程原型"的直接输入

X2Doris 的向导可归纳为 **6 屏**，DBX 可按此骨架裁剪（括号内为 DBX 建议差异）：

1. **选源** —— 树形库表，支持单表/多表/整库（DBX：整库选中后仍可展开单表审核）
2. **字段映射** —— 自动预填 + 少数几列待决策（DBX：只高亮有歧义字段，其余折叠）
3. **特殊映射** —— X2Doris 是分区映射（DBX：主键/自增/字符集/枚举等 PG 特有事项）
4. **建表意图三按钮** —— 跳过建表 / 只建表 / 建表 & 作业（**照抄**）
5. **DDL 审核** —— 全文可编辑，确认后执行（**照抄**，DBX 已定方案一致）
6. **作业设置 + 启动弹窗** —— 名称自动生成；启动时给 WHERE 过滤与"清空数据"开关（默认 OFF）

DBX 必须超越模型的三处：**表级进度 / 表级失败原因与重跑 / 界面内可读日志**。
