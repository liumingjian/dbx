...
> Method: reading source at the `3.9.0` tag (plus `4.0.0` for forward compatibility) and the KIPs. Line numbers are approximate (±5). The three open checks were later run in the lab (§4, issue #90). They sharpened the 404 half of the rule, and ADR-0032 records the mechanism as decided.
...
All three are confirmed (issue #90, 2026-09-15). Setup: the lab on a Mac mini M4 Pro, `cp-kafka-connect:7.9.0` (`7.9.0-ccs`), Compose `restart: unless-stopped`, `-XX:+ExitOnOutOfMemoryError`, no connectors deployed. Kafka 4.0.0 clients ran in `eclipse-temurin:21-jdk` on the Compose network.

1. **The marker is gone after a restart: confirmed.** The PUT returns 200 with body `["dbx.incarnation"]`, and the GET then returns `{"level":"ERROR","last_modified":<epoch ms>}`. Two crashes were run:
   - `docker kill`: Compose did **not** restart the container, because Docker treats a kill as a manual stop. After a manual `docker start`, the GET returned 502 while nothing listened (Docker Desktop's port proxy), then Connect's JSON `{"error_code":404,"message":"Logger dbx.incarnation not found."}`.
   - `kill -9` of the JVM from the VM's pid namespace (exit 137, the path an OOM exit takes): Compose restarted it within 1 s (`RestartCount` 0 → 1). The GET returned 502, then **at t+12 s an HTML Jetty 404** (`SERVLET: -`): the REST server was listening before Connect had registered its resources.

   **Rule refinement (adopted in ADR-0032):** "restarted" means Connect's JSON 404, or 200 with `last_modified: null`. A non-JSON 404 counts as unreachable, like 502. Waiting loses nothing, because the marker is a latch. A re-plant PUT in the same window also fails, so the plant retries until it gets a 200.
   - **Lab procedure:** simulate an OOM with a real process exit. `docker kill` skips the restart policy.
2. **`PUT /admin/loggers/root` rewrites the marker: confirmed.** Before the PUT, root was `{"level":"INFO","last_modified":null}`. After re-applying INFO, the marker read `{"level":"INFO","last_modified":<non-null>}`. The level changed too, not just the timestamp, so DBX must re-plant to restore `ERROR`. Under the rule this is not a restart.
3. **`describeClassicGroups` sees the Connect member, and its UUID changes: confirmed.** Group `dbx-connect` was `Stable` with 1 member, `clientId = connect-connect:8083`.
   - After `docker kill` plus a manual start, the member changed from `connect-connect:8083-d37e8ea3…` to `…-b11f511b…`. At t+45 s the group was briefly `PreparingRebalance`, and by then the old member had already expired.
   - After the `kill -9` crash, the member changed to `…-46c9cf10…`.
   - v1 does not use this signal (ADR-0032), so DBX needs no Kafka clients ≥ 4.0 for it.
